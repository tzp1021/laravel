...
function getMediaList($json_param) {
    ...
    $media = DB::select('select media.id,netSource,duration,title,iconUrl,description,album from media join channel_media on media.id = mediaId where channelId = ?', [$id]);
    $num = count($media);
    for($i = 0; $i < $num; $i++) {
